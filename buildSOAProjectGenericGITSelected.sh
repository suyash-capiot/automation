...
# ant -f $SOA_HOME/bin/ant-sca-package.xml -DcompositeDir="$baseDir"/"$LINE"/SOA -DcompositeName=$LINE -Drevision=1.0 -Dsoamds.apps.home="$mdsDir"/apps -Dscac.application.home=$baseDir -Ddeploy.composite.name=$outDir/sca_"$LINE"_rev1.0.jar 
 ...
 ant -f $SOA_HOME/bin/ant-sca-package.xml -DcompositeDir="$baseDir"/"$LINE"/SOA -DcompositeName=$LINE -Drevision=$version.0 -Dsoamds.apps.home="$mdsDir" -Dscac.application.home=$baseDir -Ddeploy.composite.name=$outDir/sca_"$LINE"_rev$version.0.jar 
...
echo "ant -f $SOA_HOME/bin/ant-sca-deploy.xml -DserverURL=$url -DsarLocation="$outDir"/sca_"$LINE"_rev"$version".0.jar -Doverwrite=true -DfailOnError=true -Duser=$userName -Dpassword=$password"

echo "=========="

ant -d -f $SOA_HOME/bin/ant-sca-deploy.xml -DserverURL=$url -DsarLocation="$outDir"/sca_"$LINE"_rev"$version".0.jar -Doverwrite=true -DfailOnError=true -Duser=$userName -Dpassword=$password
