...
do
# echo i is $i
t=`echo $i | sed 's#.*SourceCode/OSB/##g' | cut -f1 -d/ | grep -v SharedArtifacts | grep "^[a-z,A-Z]"`
# echo t is $t
...
for i in `find $WORKSPACE/OSB/ -name "*.DEV" `; do j=`echo $i | sed 's/.DEV$//g'`;`mv $i  $j`; done
echo "ant command is --- ant -f $WORKSPACE/Automation/osbDeployScript.xml -Dbasedir=$WORKSPACE/OSB -DjarDir=$tmpJARs -DsettingsFile=/$WORKSPACE/Automation/connParams.properties -Dsrcfile=$sources"
ant -f $WORKSPACE/Automation/osbDeployScript.xml -Dbasedir=$WORKSPACE/OSB -DjarDir=$tmpJARs -DsettingsFile=$WORKSPACE/Automation/connParams.properties -Dsrcfile=$sources
...
#ant -f $WORKSPACE/Automation/osbDeployScript.xml -Dbasedir=$WORKSPACE/OSB -Dsrcfile=$depfile -DjarDir=$tmpJARs -DsettingsFile=$WORKSPACE/Automation/connParams.properties write
